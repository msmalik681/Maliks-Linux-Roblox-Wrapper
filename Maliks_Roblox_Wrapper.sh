...
# download and install custom wine if missing.
if [[ $(wine --version) =~ (wine-6.15.r0.g4b6879f3) ]];
then
	echo correct wine installed!
else
	if [ ! -f ~/.wine-roblox-malik/wine-tkg-staging-fsync-git-6.15.r0.g4b6879f3-326-x86_64.pkg.tar.zst ];
	then
		cd ~/.wine-roblox-malik
		wget --no-check-certificate "https://onedrive.live.com/download?cid=CFF1642CDA1859A3&resid=CFF1642CDA1859A3%21621&authkey=AGUzM8bDM02Mi2A" -O wine-tkg-staging-fsync-git-6.15.r0.g4b6879f3-326-x86_64.pkg.tar.zst
	
	fi
	sudo pacman -U ~/.wine-roblox-malik/wine-tkg-staging-fsync-git-6.15.r0.g4b6879f3-326-x86_64.pkg.tar.zst
fi
...
# make desktop entry for roblox launcher.
echo -e "[Desktop Entry]\nVersion=1.0\nName=roblox-malik\nExec=bash -c \"WINEPREFIX=~/.wine-roblox-malik WINEFSYNC=1 wine \\\"\$(find ~/.wine-roblox-malik/drive_c/users -name 'RobloxPlayerLauncher.exe' -not -path '*/Temp/*')\\\" %U\"\nMimeType=x-scheme-handler/roblox-player;\nIcon=utilities-terminal\nType=Application\nTerminal=false\n" > ~/.local/share/applications/roblox-malik.desktop
...
then
	...
	#check if on Arch or Debian
	if [[ $(wine --version) =~ (wine-6.15.r0.g4b6879f3) ]];
	then
		echo -e "[Desktop Entry]\nName=Roblox Studio Malik\nComment=msmalik681\nExec=bash -c \"WINEPREFIX=~/.wine-roblox-malik/studio-malik WINEFSYNC=1 wine \\\"\$(find ~/.wine-roblox-malik/drive_c/users -name 'RobloxStudioLauncherBeta.exe' -not -path '*/Temp/*' -print -quit)\\\"\"\nIcon=2052_RobloxStudioLauncherBeta.0\nTerminal=false\nType=Application\nCategories=Wine;" > ~/Desktop/'Roblox Studio Malik.desktop'
		...
		WINEPREFIX=~/.wine-roblox-malik/studio-malik WINEFSYNC=1 wine "$(find ~/.wine-roblox-malik/drive_c/users -name 'RobloxStudioLauncherBeta.exe' -not -path '*/Temp/*' -print -quit)"
	else
